# Portfolio Product Brief

## Owner

**Fahmi Harun**  
GitHub: `kuker24`

## Primary objective

Create a personal portfolio that presents Fahmi as a software builder with practical experience across full-stack systems, education technology, application security, AI-assisted engineering, deployment, and developer tooling.

The website should prioritize clarity and proof of work over decorative statistics.

## Audience

- recruiters and technical reviewers
- potential collaborators
- schools or education-sector stakeholders
- prospective software clients
- developer community peers

## Personal positioning

Initial positioning:

> Full-Stack Developer, AI Engineering Educator, and Security-Focused Builder.

Supporting statement:

> Building production education platforms, privacy-first web systems, and practical developer tools.

These texts may be refined, but claims must remain supported by the project evidence.

## Featured work

### SIAB2

- Repository: `https://github.com/kuker24/abensi`
- Classification: production system
- Domain: academic information and attendance
- Initial evidence: NestJS API, React/Vite frontend, Prisma, PostgreSQL, Redis, worker reconciliation, Nginx, Docker Compose, and a secured Android QR-reader flow
- Portfolio emphasis: real operational system, full-stack architecture, deployment, attendance integrity, security, and maintainability

### SIAB1

- Repository: `https://github.com/kuker24/Ujianonline`
- Previous public repository name: Ujian Online
- Classification: production system
- Domain: digital examination
- Initial evidence: FastAPI, Flutter, PostgreSQL, Redis/Celery, Safe Exam Browser integration, WebSocket notifications, monitoring, analytics, and layered security controls
- Portfolio emphasis: secure examination workflow, asynchronous processing, cross-platform client, observability, and production architecture

### LensaDiri

- Repository: `https://github.com/kuker24/LensaDiri`
- Classification: serious hobby project
- Domain: privacy-first self-exploration
- Initial evidence: Next.js, TypeScript, PostgreSQL/Supabase tooling, row-level security, Argon2id, CSRF controls, HMAC-hashed sessions, Vitest, Playwright, and explicit scientific limitations
- Portfolio emphasis: privacy engineering, secure authentication boundaries, responsible product claims, testing, and architecture documentation

### StreamHive

- Repository: `https://github.com/kuker24/StreamLocal`
- Classification: serious hobby project
- Domain: local and LAN media streaming
- Initial evidence: Flask, HTTP Range streaming, subtitle support, QR connection, multi-user access, desktop launcher, cross-platform setup automation, and FFmpeg-related tooling
- Portfolio emphasis: local-first software, networking, desktop integration, cross-platform automation, and practical usability

## Suggested experience narrative

Use only details confirmed by the owner:

- develops and operates education-related software systems
- teaches Coding and Artificial Intelligence for grade X students
- works across frontend, backend, databases, deployment, security, and AI developer tooling
- builds both commissioned production systems and technically serious personal projects

Do not add dates, organizations, contract values, user counts, uptime, performance numbers, or client testimonials without explicit evidence.

## Initial sections

1. Hero
2. About
3. Featured projects
4. Technical capabilities
5. Experience and current focus
6. Engineering principles
7. Contact

## Visual direction

- dark or adaptive editorial interface
- strong typography and restrained accent colors
- real project evidence and screenshots as visual anchors
- minimal glass effects
- subtle motion that does not reduce readability
- no generic developer illustrations
- no excessive badge walls or GitHub-stat cards

## Required user-provided assets

These should remain explicit placeholders until supplied:

- high-resolution portrait
- preferred public email
- LinkedIn URL, when available
- downloadable CV, when available
- verified project screenshots
- concise personal biography
- confirmed experience dates and roles

## Hosting constraint

The site is hosted as a static Astro build through GitHub Pages at:

```text
https://kuker24.github.io/portfolio/
```
